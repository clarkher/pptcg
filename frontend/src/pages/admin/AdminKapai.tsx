...
interface Hit {
  ...
  marketMedian: number; sampleCount: number; profit: number; discount: number; condition: string;
}
      ...
      {hits.length > 0 && (
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead><tr>
            ...
            <th style={th}>台灣行情</th><th style={th}>賣家數</th><th style={th}>省</th><th style={th}>折扣</th><th style={th}></th>
          </tr></thead>
          <tbody>{hits.map(h => (
            <tr key={h.listingId}>
              ...
              <td style={cell}>NT${h.marketMedian.toLocaleString()}</td>
              <td style={cell}>{h.sampleCount}</td>
