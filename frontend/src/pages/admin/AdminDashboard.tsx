    ...
    <div className="space-y-8 page-enter">
      ...
      {stats ? (
        <div className="grid grid-cols-3 gap-4">
          <StatCard icon="🃏" label="上架中商品" value={stats.activeListings}
            sub={`共 ${stats.totalListings} 件`} accent="#A78BFA" />
          <StatCard icon="📋" label="待出貨訂單" value={stats.pendingOrders}
            sub={`共 ${stats.totalOrders} 筆`} accent="#FBBF24" />
          <StatCard icon="👥" label="用戶數" value={stats.totalUsers} accent="#60A5FA" />
          <StatCard icon="💰" label="累計營收" value={`NT$${stats.revenue.toLocaleString()}`}
